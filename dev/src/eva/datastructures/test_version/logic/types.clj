...
(ns eva.datastructures.test-version.logic.types
  (:require [eva.datastructures.test-version.logic.nodes :as nodes]
            ...
            [eva.datastructures.test-version.logic.tree :as tree]
            [eva.datastructures.test-version.logic.query :as query]
            [eva.datastructures.test-version.logic.error :refer [raise-safety raise-persist]]
  ...
  (:import [java.util.concurrent.atomic AtomicReference]
           [eva.datastructures.test_version.logic.nodes BufferedBTreeNode]
           [eva.v2.datastructures.bbtree.storage NodeStorage]
...
(defn- persist-structure
  ...
  (cond (not persistable?)
        (raise-persist :local-only "it is a local-only copy." {})
...
(deftype BBTreeTransientSortedMap
  ...
  (persistent [this]
    (insist-same-thread thread)
    (.set thread nil)
    (tracing "eva.datastructures.bbtree.logic.v0/persistent!"
...
(p/def-map-type BackedBBTreeSortedMap
  ...
  dsp/BackedStructure
  (storage-id [_] (if persisted? persisted-uuid (raise-safety :unpersisted-changes "No storage-id exists for this instance. Try persisting accumulated changes." {})))
  ...
  dsp/BTreeOps
  (filter [this f]
          (insist (satisfies? eva.datastructures.utils.fressian/Autological f))
...
(deftype BackedBBTreeTransientSortedMap
  ...
  dsp/BackedStructure
  (storage-id [_] (if persisted? persisted-uuid (raise-safety :unpersisted-changes "No storage-id exists for this map. Make it persistent! then call persist! first." {})))
  (store [_] store)
...
(deftype BackedBBTreeTransientSortedSet
  ...
  dsp/BackedStructure
  (storage-id [_] (if persisted? persisted-uuid (raise-safety :unpersisted-changes "No storage-id exists for this set. Make persistent! then call persist! first." {})))
  (store [_] store)
  clojure.lang.IFn
  (invoke [this k] (query/tree-get store root k))
  (applyTo [this args]
