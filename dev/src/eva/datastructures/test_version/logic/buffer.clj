...
(ns eva.datastructures.test-version.logic.buffer
  "A mappy structure with the following quirks:
  ...
  (:require [eva.datastructures.test-version.logic.protocols :refer [recip ranged? VERSION]]
            [eva.datastructures.versioning :refer [ensure-version]]
            [eva.error :refer [insist]]
            [potemkin :as p])
  (:import [clojure.lang SeqIterator]
           [eva.datastructures.protocols Versioned]))

(p/def-map-type BTreeBuffer
  [mailboxes cnt order _meta]
  (get [_ k default-value]
       (get mailboxes k default-value))
  (assoc [_ k v]
  ...
  (dissoc [_ k] (let [c (count (get mailboxes k))]
